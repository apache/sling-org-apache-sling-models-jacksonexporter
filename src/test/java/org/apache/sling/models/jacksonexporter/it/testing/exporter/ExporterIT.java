...
public class ExporterIT {
    ...
    public void testExportToJSON() throws Exception {
        try (ResourceResolver resolver = rrFactory.getServiceResourceResolver(null); ) {
            ...
            Assert.assertEquals(
                    1.0d, resource.getJsonNumber("sampleDoubleValue").doubleValue(), .1);
    ...
    public void testExportToMap() throws Exception {
        try (ResourceResolver resolver = rrFactory.getServiceResourceResolver(null); ) {
            ...
            Map<String, Object> data = modelFactory.exportModelForResource(
                    baseComponentResource, "jackson", Map.class, Collections.<String, String>emptyMap());
    ...
    public void testResourceServlets() throws Exception {
        try (ResourceResolver resolver = rrFactory.getServiceResourceResolver(null); ) {
            ...
            slingRequestProcessor.processRequest(
                    new FakeRequest(baseComponentPath + ".model.json"), response, resolver);
            JsonObject obj = Json.createReader(
                            new StringReader((response.getStringWriter().toString())))
                    .readObject();
            ...
            slingRequestProcessor.processRequest(
                    new FakeRequest(extendedComponentPath + ".model.json"), response, resolver);
            ...
            Assert.assertEquals(
                    testDate.getTimeInMillis(), obj.getJsonNumber("date").longValue());

            response = new FakeResponse();
            slingRequestProcessor.processRequest(
                    new FakeRequest(interfaceComponentPath + ".model.json"), response, resolver);
            obj = Json.createReader(new StringReader((response.getStringWriter().toString())))
                    .readObject();
            Assert.assertEquals("application/json", response.getContentType());
            Assert.assertEquals("UTF-8", response.getCharacterEncoding());
    ...
    public void testRequestServlets() throws Exception {
        try (ResourceResolver resolver = rrFactory.getServiceResourceResolver(null); ) {
            ...
            response = new FakeResponse();
            slingRequestProcessor.processRequest(
                    new FakeRequest(extendedRequestComponentPath + ".model.json"), response, resolver);
            obj = Json.createReader(new StringReader((response.getStringWriter().toString())))
                    .readObject();
            Assert.assertEquals("application/json", response.getContentType());
            Assert.assertEquals("UTF-8", response.getCharacterEncoding());
            ...
            response = new FakeResponse();
            slingRequestProcessor.processRequest(
                    new FakeRequest(interfaceRequestComponentPath + ".model.json"), response, resolver);
            obj = Json.createReader(new StringReader((response.getStringWriter().toString())))
                    .readObject();
            Assert.assertEquals("application/json", response.getContentType());
            Assert.assertEquals("UTF-8", response.getCharacterEncoding());
    ...
    public void testDoubledServlets() throws Exception {
        try (ResourceResolver resolver = rrFactory.getServiceResourceResolver(null); ) {
            FakeResponse response = new FakeResponse();
            slingRequestProcessor.processRequest(
                    new FakeRequest(doubledComponentPath + ".firstmodel.json"), response, resolver);
            ...
            response = new FakeResponse();
            slingRequestProcessor.processRequest(
                    new FakeRequest(doubledComponentPath + ".secondmodel.json"), response, resolver);
            obj = Json.createReader(new StringReader((response.getStringWriter().toString())))
                    .readObject();
            Assert.assertEquals("application/json", response.getContentType());
        ...
        try {
            try (ResourceResolver resolver = rrFactory.getServiceResourceResolver(null); ) {
                ...
                modelFactory.exportModelForResource(
                        baseComponentResource, "jaxb", String.class, Collections.<String, String>emptyMap());
